...
const CategoriesProduct = () => {
  ...
  // FIXED: Backend URL aur Image Helper
...
const getImageUrl = (images) => {
  ...
  // 1. Pehli image nikaalein (chahe array ho ya single)
  let imgData = Array.isArray(images) ? images[0] : images;

  // 2. AGAR image ek OBJECT hai (jo aapke case mein ho raha hai)
  // Check karein 'url' ya 'path' property ke liye
  let imgPath = "";
  if (typeof imgData === 'object' && imgData !== null) {
    imgPath = imgData.url || imgData.path || ""; 
  } else {
    imgPath = imgData; // Agar pehle se string hai
  }
  ...
  const subCategoryProducts = useMemo(() =>
    products.filter(p => p.subCategory?.toLowerCase() === subCategory?.toLowerCase()),
          ...
          <section className="border-t pt-6">
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between cursor-pointer">
                ...
                <span className="text-gray-400 text-xs">({subCategoryProducts.length - outOfStockCount})</span>
              </label>
              <label className="flex items-center justify-between cursor-pointer">
                <div className="flex items-center gap-3">
      ...
      <div className="w-full px-4 md:px-6 pt-5">
        ...
        <div className="flex flex-col md:flex-row h-auto md:h-[450px] gap-2 md:gap-0 w-full overflow-hidden">
          {topFiveProducts.map((cat, index) => {
            ...
            return (
              <Link to={`/product/${cat._id}`} key={cat._id} className={`relative w-full h-[150px] md:h-full md:flex-1 overflow-hidden transition-all duration-500 ease-in-out cursor-pointer group ${index !== 0 ? "md:-ml-16" : ""} ${desktopClip}`} style={{ zIndex: topFiveProducts.length - index }}>
                <img src={getImageUrl(cat.images)} alt={cat.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-110" />
              </Link>
            );
          })}
        </div>
            ...
            displayProducts.map((item) => (
              <Link to={`/product/${item._id}`} key={item._id} className="group">
                <div className="aspect-[3/4] overflow-hidden bg-gray-50 rounded-sm">
                  <img src={getImageUrl(item.images)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={item.name} />
                </div>
                <h3 className="mt-3 text-[11px] font-bold uppercase">{item.name}</h3>
                <p className="text-[10px] text-gray-500 font-medium">Rs. {item.price}</p>
              </Link>
            ))
          ) : (
