...
import { FiTrash2, FiSearch, FiUsers, FiEye } from 'react-icons/fi';
...
const UserList = () => {
  ...
  const VerifiedBadge = ({ isVerified }) => (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border font-bold uppercase text-[9px] tracking-wider ${
      ...
      {isVerified ? "Verified Buyer" : "Unverified"}
    </span>
  );
        ...
        <header className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight italic text-[#1E1B4B]">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {filteredUsers.map((user) => (
                ...
                >
                  {/* Avatar */}
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-base shadow-sm tracking-wide shrink-0 ${getAvatarStyle(user.name)}`}>
                    {(user.name || "?").charAt(0).toUpperCase()}
                  </div>

                  {/* Identity */}
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-gray-700 text-sm tracking-tight truncate">
                      {user.name || "Unnamed"}
                    </p>
                    <p className="text-[11px] text-gray-500 font-normal truncate select-all">
                      {user.email}
                    </p>
                ...
                {loading ? (
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user._id} className="hover:bg-slate-50/50 transition-all group">
                      ...
                      <td className="p-4 pl-6 max-w-[260px]">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <span className="font-bold text-gray-700 text-sm tracking-tight truncate">
                            {user.name || "Unnamed"}
                          </span>
                        </div>
                      </td>
